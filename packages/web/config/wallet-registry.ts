...
import { RegistryWallet, WalletConnectionInProgressError } from "@osmosis-labs/stores";
...
export const WalletRegistry: RegistryWallet[] = [
  {
    ...
    lazyInstall: () => import("@cosmos-kit/keplr-extension").then((m) => m.KeplrExtensionWallet),
  ...
  },
  {
    ...
    logo: "/wallets/keplr.svg",
    lazyInstall: () => import("~/integrations/keplr-walletconnect").then((m) => m.KeplrMobileWallet),
  ...
  },
  {
    ...
    mobileDisabled: false,
    lazyInstall: () => import("@cosmos-kit/leap-extension").then((m) => m.LeapExtensionWallet),
  ...
  },
  {
    ...
    mobileDisabled: false,
    lazyInstall: () => import("@cosmos-kit/namada-extension").then((m) => m.NamadaExtensionWallet),
    windowPropertyName: "namada",
    stakeUrl: "https://namada.net/",
    governanceUrl: "https://namada.net/",
    features: ["notifications"],
  },
  {
    ...CosmosKitWalletList["leap-cosmos-mobile"],
    logo: "/wallets/leap.svg",
    lazyInstall: () => import("@cosmos-kit/leap-mobile").then((m) => m.LeapMobileWallet),
    supportsChain: async (chainId) => {
  ...
  },
  {
    ...
    lazyInstall: () => import("@cosmos-kit/cosmostation-extension").then((m) => m.CosmostationExtensionWallet),
  ...
  },
  {
    ...
    lazyInstall: () => import("@cosmos-kit/xdefi-extension").then((m) => m.XDEFIExtensionWallet),
  ...
  },
  {
    ...
    lazyInstall: () => import("@cosmos-kit/station-extension").then((m) => m.StationExtensionWallet),
  ...
  },
  {
    ...
    lazyInstall: () => import("@cosmos-kit/okxwallet-extension").then((m) => m.OkxwalletExtensionWallet),
    ...
    async supportsChain(chainId, retryCount = 0) {
      ...
      try {
        ...
        if (error.code === -32603 && error.message.includes("There is no chain info")) {
          return false;
        }

        // Retry if the wallet is already processing
        if (error.code === -32002 && error.message.includes("Already processing") && retryCount < 5) {
          /**
    ...
    matchError: (error) => {
      ...
      if (error.includes("Already processing wallet_requestIdentities. Please wait.")) {
